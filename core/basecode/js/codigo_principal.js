...
function form_supervisionado(svalor){
	if(svalor == "sim"){ $("p#supervisionadosim").css({color: "green"}); $("p#supervisionadonao").css({color: "#999999"});
	} else if(svalor == "nao"){ $("p#supervisionadosim").css({color: "#999999"}); $("p#supervisionadonao").css({color: "green"}); }
}
...
function categoriasub(dados) {
	...
	function buscacategoriasub(dados){
		$.post("ajax.php", {    acao: "subordinado",
			   					subordinadoid: dados
							}, function(txt){
								//alert(txt);
								if(txt > 0){
...
function alreadyexists(svalor, scampo, iftrue, iftruecolor, iffalse, iffalsecolor, inwhatdbtable){
    if(svalor != ""){
		$.post("ajax.php", {    acao: "verifyifexists",
								valor: svalor,
							    campo: scampo,
